...
import FData from './models/FData.js'; //מייבאים את מבנה הנתונים בקובץ המודל שהשרת ידע אילו שדות וטווחים קיימים
...
mongoose.connect(process.env.MONGO_URI)
    .then(() => console.log("connection to Mongo via docker successful")) // פעולה אסינכרונית שימוש בםרומיס והדפסה במידה ומצליח
    .catch((err) => console.error('connection to Mongo Failed:', err)); // אם לא מצליח תופסים את השגיאה והיא מודפסת באדום
// יצירת נתיב בדיקה
app.get('/test', (req, res) => {  // הרק מכילה את כל המידע שהגיע מהדפדפן והרס מכיל פונקציות שמחזירים איתן תשובה
    res.send('server running'); // שימוש בפונקציית שליחה לשלוח טקסט פשוט
});

app.get('/preparing', (req,res) => {
    res.send(`you've prepared`);
})
...
app.get('/api/indicators', async (req, res) => { // פונקציה אסינכרונית 
    try { //קוד הגנה על מנת שאם משהו ישתבש  יעבור לקאץ
        ...
        const latestData = await FData.findOne().sort({ createdAt: -1 });
...
app.post('/api/indicators', async (req, res) => { 
    try { // שכבת הגנה
        ...
        const newData = new FData({ altitude, hsi, adi }); // מייצרים מסמך חדש על פי המודל נתונים שלנו מהנתונים ששלפנו
...
app.listen(PORT, '0.0.0.0', () => { //הפעלת השרת והאזנה לבקשות בפורט שהצבנו ובכתובת פרטית שניתן לגשת ממנה מכל מחשב ברשת הפרטית
    ...
    console.log(`access in private network`);
});